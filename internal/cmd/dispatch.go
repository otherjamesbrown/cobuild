...
func writePhasePrompt(b *strings.Builder, phase, workItemID, taskID string, pCfg *config.Config) {
	...
	case "decompose":
		...
		b.WriteString("4. Order by dependency — assign wave numbers (wave 1 has no blockers, wave 2 depends on wave 1)\n")
		b.WriteString("5. For each task, create a work item in the **correct target project** with scope, acceptance criteria, and code locations:\n")
		b.WriteString(fmt.Sprintf("   `cobuild wi create --project <target-project> --type task --title \"<title>\" --body \"<spec>\" --parent %s`\n", workItemID))
		b.WriteString("6. For each task, set `repo` metadata to the target repo name:\n")
		b.WriteString("   `cxp shard metadata set <task-id> repo <repo-name>`\n")
		b.WriteString("7. Link dependencies between tasks:\n")
		b.WriteString("   `cobuild wi links add <task-id> <blocker-id> blocked-by`\n")
		b.WriteString("8. Record the decomposition gate with a summary that includes the spec → project/repo mapping:\n")
		b.WriteString(fmt.Sprintf("   `cobuild decompose %s --verdict pass --body \"<summary with spec-to-project-repo map>\"`\n", workItemID))
		b.WriteString("9. **Type `/exit` to end your session.** The orchestrating agent will dispatch implementation. Do NOT dispatch tasks yourself.\n\n")
