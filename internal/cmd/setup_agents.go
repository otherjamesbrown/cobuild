...
func generateAgentsContent(project, prefix string, workflows map[string]string, skills map[string][]string, pCfg *config.Config) string {
	...
	sb.WriteString("| `cobuild dispatch <task-id>` | Dispatch a task to a dispatched CoBuild agent |\n")
	...
	sb.WriteString("### Option B: Delegate to a dispatched CoBuild agent\n\n")
	sb.WriteString("If you want a dispatched CoBuild agent to handle a phase in its own context:\n\n")
	sb.WriteString("```bash\n")
	sb.WriteString("cobuild dispatch <id>   # spawns dispatched CoBuild agent in tmux for the current phase\n")
	sb.WriteString("cobuild wait <id>       # blocks until the dispatched CoBuild agent completes\n")
	sb.WriteString("```\n\n")
	sb.WriteString("`cobuild dispatch` is phase-aware — it generates the right prompt automatically.\n")
	sb.WriteString("Use this for implementation (dispatched CoBuild agents write code) and when you want a clean context.\n\n")
	...
	sb.WriteString("| You need a dispatched CoBuild agent to write code | Option B — dispatch |\n")
	...
	sb.WriteString("cobuild dispatch <fix-task-id>               # spawns dispatched CoBuild agent (implement skill)\n")
	...
	sb.WriteString("cobuild dispatch <task-id>                   # spawns dispatched CoBuild agent (implement skill)\n")
