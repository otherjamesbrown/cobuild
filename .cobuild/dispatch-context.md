# Task: Auto-create pipeline run on direct dispatch
...
## Task B: Auto-create pipeline run on direct dispatch
...
Make `cobuild dispatch <id>` work without requiring a prior `cobuild init`. Auto-create a pipeline_run on the fly if one does not exist. This fixes gate commands (`cobuild investigate`, `cobuild review`, etc.) failing for directly-dispatched work items.

## Changes

1. `internal/cmd/dispatch.go` — after loading the work item, before building the prompt:
   - Call `cbStore.GetRun(ctx, task.ID)`
   - If no run exists, create one via `cbStore.CreateRunWithMode`:
     - `design_id = task.ID`
     - `project = projectName`
     - `workflow = inferWorkflowFromType(task.Type)` — bug → bug, design → design, task → task
     - `current_phase = firstPhaseOf(workflow)` from pipeline config
     - `status = "active"`
     - `mode = "manual"`
   - Print a one-line notice: `Auto-created pipeline run for <id> (workflow: <wf>, phase: <phase>)`
2. Add helper `inferWorkflowFromType(wiType string) string` in `internal/cmd/dispatch.go` or `internal/config/config.go`
3. Add helper `firstPhaseOf(workflow string, cfg *Config) string` that looks up the first phase of a workflow from the loaded config, with a sensible fallback

## Acceptance criteria

- [ ] `cobuild dispatch <id>` succeeds when no pipeline run exists (creates one)
- [ ] A one-line notice is printed when auto-creation happens
- [ ] The auto-created run has the correct workflow, first phase, and mode=manual
- [ ] `cobuild investigate <bug-id>` succeeds after `cobuild dispatch <bug-id>` (no more "no pipeline run" errors)
- [ ] `cobuild init` still works as before and is idempotent with the auto-create path (running init after auto-create should detect the existing run and not duplicate)
- [ ] `go build ./...` and `go vet ./...` pass
- [ ] Integration check: dispatch a bug directly, then run a gate command — confirm it works end-to-end

## Out of scope

- Changing the set of phases or workflows (handled by Task C/D)
- Deprecating `cobuild init` — it still has value for explicit workflow selection and batch setup
...
1. **Run `cobuild complete cb-332869`** -- this commits remaining changes, pushes, creates the PR, appends evidence, and marks the task needs-review. Do this as your LAST action.
